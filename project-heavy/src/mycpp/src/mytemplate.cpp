 ...
 *
 *  Versioning: https://github.com/timflutre/tuto-reproducible-research
 *
 *  Compile with: g++ -Wall -g utils_io.cpp mytemplate.cpp -lgsl -lgslcblas -lz -o mytemplate
...
void help(char ** argv)
{
  cout << "`" << argv[0] << "'"
       ...
       << "  " << argv[0] << " -i <input>" << endl
       << endl
       << "Report bugs to <mycpp-users@...>." << endl
    ;
}

/** \brief Display version and license information on stdout.
 *  \note The person roles comply with R's guidelines (The R Journal Vol. 4/1, June 2012).
 */
void version(char ** argv)
{
  cout << argv[0] << " " << VERSION << endl
       << endl
       << "Copyright (C) 2011-2015 Timothée Flutre." << endl
       << "License GPLv3+: GNU GPL version 3 or later <http://gnu.org/licenses/gpl.html>" << endl
       << endl
       << "Written by Timothée Flutre [cre,aut]." << endl
    ;
}
  ...
  int & verbose)
{
  ...
  if(input.empty()){
    cerr << "ERROR: missing compulsory option --input" << endl << endl;
    help(argv);
    exit(1);
  }
  if(! doesFileExist(input)){
    cerr << "ERROR: can't find file " << input << endl << endl;
    help(argv);
    exit(1);
  }
}
